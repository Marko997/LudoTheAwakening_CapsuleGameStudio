...
//////                    float4 d = fwidth(outline);
//////                    float4 alpha = saturate((d - _OutlineWidth) / d);
//////                    outline.a = alpha.r * alpha.g * alpha.b * alpha.a;
...
Shader "Custom/NeonGlowShader" {
    Properties{
        ...
        _GlowWidth("Glow Width", Range(0.0, 1.0)) = 0.1
    }
        ...
        SubShader{
            ...
            Pass {
                ...
                fixed4 frag(v2f i) : SV_Target {
                    // Apply the emission effect
                    fixed4 col = tex2D(_MainTex, i.uv) * _EmissionColor;

                // Compute the distance between the pixel and the object's world position
                float distance = length(i.worldPos - _WorldSpaceCameraPos);

                // Apply the glow effect
                float glow = pow(1.0 - distance * _GlowIntensity, 3.0);
                float alpha = pow(saturate((_GlowWidth - distance) / _GlowWidth), 3.0);
