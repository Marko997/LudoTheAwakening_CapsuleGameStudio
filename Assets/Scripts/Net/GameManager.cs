...
public class GameManager : NetworkBehaviour
{
    ...
    [SerializeField] private LudoTile[] board;
    ...
    private const int TILE_COUNT = 56;
    ...
    private void Awake()
    {
        ...
        {
            //board[i] = gameObject.AddComponent<LudoTile>();
    ...
    private void Update()
    {
        if (playersSpawned)
        {
            ...
            if (IsServer)
            {
                ...
                {
                    playerReady.Add(netClient.Key, false);
                    var player = netClient.Value.PlayerObject.GetComponent<PlayerController>();
                    if (player.IsOwner)
                    {
                        // If this is our player, use that to assign the UI
                        playerColor.color = Utility.TeamToColor(((Team)Utility.RetrieveTeamId(player.OwnerClientId)));
        ...
        if (currentTurn.Value == NetworkManager.Singleton.LocalClientId)
        {
            ...
            if (Physics.Raycast(ray, out hit, 50.0f, LayerMask.GetMask("ActivePiece")))
            {
                ...
                {
                    //hit.collider.GetComponent<Piece>().isSelected = true;
                    Piece selectedPiece = hit.collider.GetComponent<Piece>();
                    MovePiece(selectedPiece);
                    if (!selectedPiece.isMoving && selectedPiece.isOut)
                    {
    ...
    {
        // 0. Are we in play? if not, did we roll a 6   piece.currentTile == -1 && 
    ...
    private void SpawnAllPlayers()
    {
        ...
        foreach (KeyValuePair<ulong, NetworkClient> nc in NetworkManager.Singleton.ConnectedClients)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                boardList.RemoveAt(51); //Remove unneeded tile
                pieces[i].board = boardList.ToArray();
    ...
    private void EnterPieceServerRpc(int index, ulong clientIndex)
    {
        ...
        //piece.isOut = true; //ADD THIS AFTER TESTING
    }
    [ServerRpc(RequireOwnership = false)]
    private void MovePieceServerRpc(int indexPos, ServerRpcParams serverRpcParams = default)
    {
        ...
        board[piece.currentTile].RemovePiece(piece); //Remove piece from start position when piece leave house
        ...
        piece.steps = currentDiceRoll.Value; //ads steps to pawn
